...
export default async function NewsDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Tăng lượt xem (Server Action đơn giản hoặc tự gọi API route)
      ...
      <main className="flex-1 pb-24">
            ...
            <div className="container mx-auto px-4 pb-16 max-w-[900px]">
              ...
              <div className="flex flex-wrap items-center gap-6 text-slate-300 text-sm font-medium">
                <div className="flex items-center gap-2">
                  ...
                  <span>{post.author?.email || 'Ban biên tập GNP'}</span>
                </div>
        ...
        <div className="container mx-auto px-4 max-w-[900px] -mt-10 relative z-10">
          <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] p-6 md:p-10 lg:p-14 border border-slate-100">
            ...
            {/* Share Buttons */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10 pb-8 border-b border-slate-100">
              <span className="text-slate-500 font-semibold text-sm flex items-center gap-2 uppercase tracking-wider">
                <Share2 className="w-4 h-4" /> Chia sẻ bài viết
              </span>
              <div className="flex gap-3">
                <button className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-600 hover:bg-[#1877F2] hover:text-white transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5">
                  <Facebook className="w-4 h-4" />
                </button>
                <button className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-600 hover:bg-[#1DA1F2] hover:text-white transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5">
                  <Twitter className="w-4 h-4" />
                </button>
                <button className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-600 hover:bg-[#0A66C2] hover:text-white transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5">
